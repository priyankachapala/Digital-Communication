...
for ii=1: length(ebn0db)
    rxd1 = si + 10^(-ebn0db(ii)/20)*nn;
    rxd = real(rxd1)>0;
    nerr(ii)=size(find(ip-rxd),2);
    coded_n = (2*coded-1) + 10^(-ebn0db(ii)/20)*nn1;
...
uncodber = nerr/N;
thber = 0.5*erfc(sqrt(10.^(ebn0db/10)));
figure;
semilogy(ebn0db,BER,'k--','Linewidth',1);
hold on;
% semilogy(ebn0db,uncodber,'b*-','Linewidth',1);
% hold on;
semilogy(ebn0db,thber,'r+-','Linewidth',1);
hold on;
% legend('Coded AWGN','Uncoded AWGN','THEORETICAL BER');
legend('Coded AWGN','THEORETICAL BER');
title('BER PLOTS OF CODED AND THEORETICAL BPSK');
